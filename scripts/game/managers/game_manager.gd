...
func set_hero_quest(quest: Quest):
	_hero_quest = quest
	Utils.connect_signal(_hero_quest, "completed", self, "_end_game", [true], CONNECT_ONE_SHOT)
	_hero_quest.begin_tracking()


func set_overlord_quest(quest: Quest):
	_overlord_quest = quest
	Utils.connect_signal(_overlord_quest, "completed", self, "_end_game", [false], CONNECT_ONE_SHOT)
	_overlord_quest.begin_tracking()


func _end_game(_quest, was_hero_quest: bool):
